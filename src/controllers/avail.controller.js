...
export const getAvailability = catchAsync(async(req,res,next) => {
  ...
  if(!req.params.staffId){
    return next(new AppError('Staff Id is missing','parameters are missing',400))
  }
...
export const updAvailability = catchAsync(async (req,res,next) => {
  ...
  if(!req.params.staffId){
    return next(new AppError('Staff Id not found','parameters are missing',400))
  }
  ...
  await Availability.findOneAndUpdate({staffId: req.params.staffId},req.body)
