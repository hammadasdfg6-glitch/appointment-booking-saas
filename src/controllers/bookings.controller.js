...
export const createBooking = catchAsync(async (req,res,next) => {
    ...
    const service = await Service.findOne({_id: serviceId,orgId,active: true})
    ...
    const slot = chkslot.slots.find((item) => {
        ...
        if(timeToMinutes(item.startTime) >= timeToMinutes(startAt) && timeToMinutes(item.endTime) >= timeToMinutes(endAt) && 'available' === item.status){
            return item
        }
    })
    ...
    const booking = new Booking({orgId,staffId,customerId,serviceId,slotId,status,date,price,startAt: slot.startTime,endAt: slot.endTime})
    await booking.save()
    await chkslot.save()
    ...
    await emailJob(staff.email, 'New Booking', 'You have a new appointment scheduled!')
    ...
    if (0 < delay) {
        await emailJob(customer.email, 'Reminder: Upcoming Appointment', `Don't forget! Your appointment is tomorrow at ${slot.startTime}.`, { delay })
    }
...
export const getBookings = catchAsync( async (req,res,next) => {
    ...
    let expiryTime = 30 
    if (_id) expiryTime = 360 
    else if (date) expiryTime = 100 
    else if (staffId) expiryTime = 45 
    else if (customerId || status) expiryTime = 40 
    ...
    await redis.set(cacheKey, JSON.stringify(responseData), 'EX', expiryTime + (Math.floor(Math.random() * 10) + 1))
...
export const deleteBooking = catchAsync( async (req,res,next) => {
    ...
    if (req.user.role === 'customer' && booking.customerId.toString() !== req.user._id.toString()) {
        return next(new AppError('You can only cancel your own bookings', 'Forbidden', 403));
    }
    ...
    if (customer) await emailJob(customer.email, 'Booking Cancelled', 'Your booking has been cancelled.')
    if (staff) await emailJob(staff.email, 'Booking Cancelled', 'An appointment has been cancelled.')
...
export const setStatus = catchAsync( async (req,res,next) => {
    ...
    const customer = await User.findOne({_id: booking.customerId}, 'email')
    if (customer) await emailJob(customer.email, 'Booking Status Update', `Your booking status is now: ${status}`)
